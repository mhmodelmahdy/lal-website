...
import { Package, FileText, CheckCircle, Send, ClipboardList, AlertCircle } from "lucide-react";
...
export default function SuppliersPortalClient({ lang }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
          ...
          if (data.success) {
            setSent(true);
            setForm({ company: "", contact: "", contactInfo: "", products: "", message: "" });
            setTimeout(() => setSent(false), 5000);
          } else {
            setError(data.error || t("حدث خطأ أثناء التسجيل", "Registration failed"));
          }
        } catch (err) {
          setError(t("خطأ في الاتصال بالخادم", "Connection error"));
        } finally {
          setLoading(false);
        }
    };
                    ...
                    <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h3 className="text-lg font-bold text-navy-dark">
                                {t("متطلبات التسجيل", "Registration Requirements")}
                            </h3>
                        </div>
                    ...
                    <div className="bg-white border border-gray-100 rounded-2xl p-6 md:p-8 shadow-sm">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h2 className="text-2xl font-bold text-navy-dark">
                                {t("نموذج تسجيل مورد", "Supplier Registration Form")}
                            </h2>
                        </div>
                        ...
                        {sent && (
                            ...
                            >
                                ...
                                <span className="text-sm">
                                    {t("تم التسجيل بنجاح! سنتواصل معك قريباً", "Registration successful! We'll contact you soon")}
                                </span>
                            </motion.div>
                        )}
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <input 
                                ...
                                className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-navy focus:ring-2 focus:ring-navy/10 outline-none transition text-sm"
                                placeholder={t("معلومات التواصل (رقم الهاتف أو البريد الإلكتروني)", "Contact Info (Phone or Email)")} 
                            />
                            ...
                            <motion.button 
                                ...
                                className="w-full px-6 py-4 bg-gradient-to-r from-navy to-navy-dark text-white font-bold rounded-xl hover:shadow-lg transition-shadow flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
                            >
                                {loading ? (
                                    <>
